...
| Stop speech | `Esc` while the assistant is talking (only grabbed then; other apps keep Esc), or say "stop", "quiet", "never mind" |
...
The mic stays open, with macOS echo cancellation so it can work on speakers. Three ways to cut the voice off:
talk over it (it stops and listens), press `Esc` (it stops and waits), or say a short "stop" (it stops without replying).
A longer sentence such as "stop the dev server" is treated as a request, not an interrupt.
