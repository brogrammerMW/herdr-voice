...
# the 16:9 and 9:16 clips with the house track (YouTube and X; Shorts, Reels and TikTok). Needs Swift, ffmpeg and img2webp (brew install webp).
set -eu
cd "$(dirname "$0")"
assets=../../assets
work=$(mktemp -d)
...
mkdir -p "$work/square" "$work/wide" "$work/tall" "$work/preview" "$assets"

"$work/render" "$work/square"
"$work/render" "$work/wide" --wide
"$work/render" "$work/tall" --tall
...
ffmpeg -v error -y -framerate 30 -i "$work/tall/f%04d.png" -i "$work/music.wav" $x264 -profile:v high \
    -c:a aac -b:a 192k -shortest "$assets/herdr-voice-launch-9x16.mp4"
